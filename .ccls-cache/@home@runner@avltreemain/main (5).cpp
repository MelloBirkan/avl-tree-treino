...
public:
    ...
    Node(Produto p) : produto(p), left(nullptr), right(nullptr), parent(nullptr), height(1) {}
};
...
void displayLessThanQuantidade(int quantidade, Node* node) {
    ...
    Node* search(int codigo) {
        return searchNode(root, codigo);
    }
...
private:
    ...
    void printAllProducts(Node* node) {
        ...
        cout << "codigo(" << node->produto.getCodigo() << ") "
             //<< "antecessor:" << (node->parent != nullptr ? node->parent->produto.getCodigo() : -1) << " "
             << "filho_esquerda:" << (node->left != nullptr ? node->left->produto.getCodigo() : -1) << " "
             << "filho_direita:" << (node->right != nullptr ? node->right->produto.getCodigo() : -1) << " "
             << "Degree: " << (node->left != nullptr ? 1 : 0) + (node->right != nullptr ? 1 : 0) << " "
             << "nivel: " << getLevel(node) << " "
             << "Height: " << node->height << " "
...
void displayTree(Node* node, int indent) {
    ...
    if (node->right != nullptr) {
        cout << string(indent, ' ') << "R: " << node->right->produto.getCodigo() << endl;
    }

    if (node->left != nullptr) {
        cout << string(indent, ' ') << "L: " << node->left->produto.getCodigo() << endl;
    }
 ...
 double calculateTotalValue(Node* node) {
        ...
        double total = node->produto.getQuantidade() * node->produto.getValorUnitario();
...
while (true) {
    ...
    switch (choice) {
        ...
        case 2: {
            ...
            cout << "Produto encontrado na posicao " << node->produto.getCodigo() << endl;
            cout << "Dados do produto: " << endl;
            cout << "Codigo: " << node->produto.getCodigo() << endl;
            cout << "Nome: " << node->produto.getNome() << endl;
            cout << "Valor unitario: R$" << node->produto.getValorUnitario() << endl;
            cout << "Quantidade em estoque: " << node->produto.getQuantidade() << endl;
            cout << "Valor total do item: R$" << node->produto.getQuantidade() * node->produto.getValorUnitario()<< endl;
            cout << "Filho esquerda: " << (node->left ? to_string(node->left->produto.getCodigo()) : "null") << endl;
            cout << "Filho direita: " << (node->right ? to_string(node->right->produto.getCodigo()) : "null") << endl;
            break;
        }
        case 3: {
        ...
        case 6: {
            ...
            cout << "Percentagem em relacao ao total do estoque:  "<< node->produto.getQuantidade() * node->produto.getValorUnitario()/ totalValue * 100<< "%"<<endl;
